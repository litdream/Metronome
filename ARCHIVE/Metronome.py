...
def main():
    ...
    sa = "hs:t:d:f:"
    la = ('help', 'dryrun', 'sample=', 'tempo=', 'duration=', 'file=' )
    ...
    for k,v in o:
        ...
        elif  k in ('-f', '--file')     : filename = v.strip()
        elif  k == '--dryrun'           : dryrun = True

    if tempo < 40 or tempo > 250:
        print( "Invalid tempo (%d):    Make it between 40 - MAX" % tempo, file=sys.stderr)
        print( "   * longer the sample length, smaller the MAX", file=sys.stderr)
        sys.exit(1)
